...
def save_to_excel_with_performance_summary(
    ...
    df_X_num = pd.DataFrame(
        X_num_np,
        columns=[f"NumFeature_{i}" for i in range(X_num_np.shape[1])]
        )
    df_X_img = pd.DataFrame(
        X_img_np.reshape(X_img_np.shape[0], -1),
        columns=[f"ImgFeature_{i}" for i in range(
            X_img_np.size // X_img_np.shape[0]
            )])
    ...
    with pd.ExcelWriter(filename) as writer:
        df_X_num.to_excel(
            writer,
            sheet_name="Numerical Features",
            index=False
            )
        df_X_img.to_excel(
            writer,
            sheet_name="Image Features",
            index=False)
        df_y.to_excel(
            writer,
            sheet_name="Targets",
