...
class FeatureSelectionPage:
    ...
    def __init__(
            ...
            X_img=None,
            y=None,
            X_num_val=None,        # <-- ADICIONADO
            X_img_val=None,        # <-- ADICIONADO  
            y_val=None,            # <-- ADICIONADO
            save_results_callback=None):
        ...
        self.X_num_val_tensor = X_num_val        # <-- ADICIONADO
        self.X_img_val_tensor = X_img_val        # <-- ADICIONADO
        self.y_val_tensor = y_val                # <-- ADICIONADO
        ...
        if self.X_num_val_tensor is None and self.X_img_val_tensor is None:
            st.error("Dados de validação (X_num_val ou X_img_val) não fornecidos para FeatureSelectionPage.")
            return
        if self.y_val_tensor is None:
            st.error("Labels de validação (y_val) não fornecidos para FeatureSelectionPage.")
            return
        ...
        if self.X_num_tensor is not None:
            st.info(f"📊 Dados de treino: {self.X_num_tensor.shape[0]} amostras")
        if self.X_num_val_tensor is not None:
            st.info(f"📊 Dados de validação: {self.X_num_val_tensor.shape[0]} amostras")
        ...
        if self.n_features_numerical > 0 or self.use_image_for_evaluation:
            with st.spinner(f"GA is optimizing features... (Generations: {num_gen}, Population: {num_pop})"):
                ga_raw_results_dict = genetic_algorithm(
                    ...
                    X_img_tensor=self.X_img_tensor,
                    y_tensor=self.y_tensor,
                    X_num_val=self.X_num_val_tensor,        # <-- ADICIONADO
                    X_img_val=self.X_img_val_tensor,        # <-- ADICIONADO
                    y_val=self.y_val_tensor,                # <-- ADICIONADO
                    fitness_fn=evaluate_features,
                    n_features=self.n_features_numerical,
                    n_population=int(num_pop),
                    n_generations=int(num_gen),
            ...
            st.write(
                f"GA Best Fitness (Validation Accuracy): {ga_results['best_fitness']:.4f}")  # <-- ALTERADO
            st.write(
                f"Number of selected numerical features (GA): {ga_selected_numerical_count}")
            if self.use_image_for_evaluation:
                st.write(f"Image data was considered in GA evaluation.")
            ...
            st.write(
                f"GA-selected numerical features (binary mask): {ga_selected_numerical_mask.astype(int)}")
            st.info("🎯 Fitness calculado usando dados de validação para evitar overfitting")  # <-- ADICIONADO
        else:
            st.warning(
                "GA not run: No numerical features to select and image data not available/used for evaluation context.")
        ...
        if self.n_features_numerical > 0 or self.use_image_for_evaluation:
            ...
            st.write(
                f"PSO Best Fitness (Validation Accuracy): {pso_results['best_fitness']:.4f}")  # <-- ALTERADO
            st.write(
                f"Number of selected numerical features (PSO): {pso_selected_numerical_count}")
            if self.use_image_for_evaluation:
                st.write(f"Image data was considered in PSO evaluation.")
